package com.liningkang.ui.view.lcardview
...
class LCardView @JvmOverloads constructor(
    context: Context,
    ...
    private val defaultShadowColor = Color.parseColor("#000000")
    ...
    init {
        ...
        val typedArray =
            context.theme.obtainStyledAttributes(attrs, R.styleable.LCardView, defStyleAttr, 0)
        ...
        for (i in 0 until indexCount) {
            when (val index = typedArray.getIndex(i)) {
                ...
                R.styleable.LCardView_card_shadowColor -> {
        ...
        when (widthMode) {
            MeasureSpec.AT_MOST, MeasureSpec.EXACTLY -> {
                ...
                wms = MeasureSpec.makeMeasureSpec(
                    max(heightMode, MeasureSpec.getSize(wms)),
                    widthMode
                )
                ...
                when (heightMode) {
                    MeasureSpec.AT_MOST, MeasureSpec.EXACTLY -> {
                        ...
                        hms = MeasureSpec.makeMeasureSpec(
                            max(minHeight, MeasureSpec.getSize(hms)),
                            heightMode
                        )
                        super.onMeasure(
                            if (isFixedContentWidth) 0 else wms,
                            if (isFixedContentHeight) 0 else hms
                        )
                    }
...
//        canvas.restore()
    ...
    private fun isSameRGB(color: Int): Boolean {
        return if (shadowColor == color) {
            true
        } else Color.red(color) == Color.red(shadowColor) && Color.green(color) == Color.green(
            shadowColor
        ) && Color.blue(color) == Color.blue(shadowColor)
    }
    ...
    private fun initColors(@ColorInt color: Int) {
        ...
        shadowColor = Color.argb(
            if (elevationAffectShadowColor) cardElevation + 10 else shadowAlpha,
            red,
            green,
            blue
        )
    ...
    private fun onShadowSizeChange() {
        ...
        val needReCreateDrawable =
            (paddingRight == 0 || rightPadding == 0) || (paddingLeft == 0 || leftPadding == 0)
